...
- Distributions from the Peixão dataset (Peixao_2000_alkanes). 
...
- Ensure MATLAB (R20XX or later) is installed with required toolboxes (if applicable).
...
- Requires the external function `NMF.m` from Polissar: Polissar, P; Karp, A. Tyler; D'Andrea, William (2025), “Mixed messages: Unmixing sedimentary molecular distributions reveals source contributions and isotopic values”, Mendeley Data, V2, doi: 10.17632/3hymgv47jv.2. The `NMF.m` file must be downloaded separately and placed in the same directory as the script or added to the MATLAB path, which must be available in the MATLAB path.
